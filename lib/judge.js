    ...
    judge = (function () {
        return {
            ...
            browser: function () {
                ...
                var
                    //IE版本号判断
                    IEVersion = (ua.indexOf('msie'))!=-1 ? parseInt(ua.split('msie')[1]):false,
                    ...
                    //避免只匹配browser而引发的冲突
                    mobileBaidu = ua.match(/baidubrowser/)=='baidubrowser',
                    MQQBrowser = ua.match(/qqbrowser/) && ua.match(/android/),
                    QQBrowser = ua.match(/qqbrowser/)=='qqbrowser',
                    Opera = ua.indexOf('opr') > -1,
                    MIUIBrowser = ua.match(/miuibrowser/) == 'miuibrowser',
                    OppoBrowser = ua.match(/oppobrowser/) == 'oppobrowser',

                    //匹配mobile safari
                    mobileSafari = ua.match(/mobile safari/) == 'mobile safari'&&
                                   ua.match(/iphone/),

                    //androidchrome和ios chrome的ua不一样，需做不同处理
                    AndroidChrome = ua.match(/android/) && ua.match(/chrome/),
                    iosChrome = (ua.match(/iphone/) || ua.match(/ipad/)) && ua.match(/crios/) =='crios',
                    Chrome = ua.indexOf('chrome') > -1,
                    Safari = ua.match(/safari/i) && ua.indexOf('chrome') < 1;
                var browsers = [
                    IEMobile,Firefox, Edge,Sougou,Liebao,lbmobile,Weixin,UC,mobileUC,
                    mobileBaidu,MQQBrowser,QQBrowser,Opera,MIUIBrowser,OppoBrowser,
                    mobileSafari,AndroidChrome,iosChrome,Chrome,Safari
                ];
                //bn => "Browser Name"
                var bn = [
                    "Mobile IE", "Firefox", "Edge","Sougou","Liebao","Liebao Mobile","Weixin",
                    "UC","Mobile UC","Mobile Baidu","Mobile QQBrowser","QQBrowser","Opera",
                    "MIUI Browser","Oppo Browser","Mobile Safari","Android Chrome",
                    "iOS Chrome","Chrome", "Safari"
                ];

                for(var i = 0,l= browsers.length;i < l;i++){
                    if(browsers[i]&&!IEVersion){
                        return bn[i];
                    }
                    if(IEVersion){
                        return "IE"+IEVersion;
